  ...
  return (
    <Link href={`/projects/${slug}`} className="h-full">
      <div className="group relative flex h-full flex-col overflow-hidden rounded-xl border border-gray-800 bg-gray-900/50 shadow-lg backdrop-blur-sm transition-all hover:shadow-2xl hover:shadow-purple-500/10">
        <div className="relative h-44 w-full overflow-hidden bg-gray-800 sm:h-56 md:h-60">
          <Image
        ...
        <div className="flex h-full flex-col justify-between gap-3 p-5 sm:gap-4 sm:p-6">
          <div>
            <h3 className="mb-1 text-lg font-bold text-gray-100 sm:mb-2 sm:text-xl">{title}</h3>
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag) => (
              <span
                key={tag}
                className="rounded-full bg-gray-800 px-3 py-1 text-xs text-gray-300 sm:text-sm"
              >
                {tag}
              </span>
            ))}
